...
model.add(layers.Dense(6,name='output'))
...
#绘制预测图像
flower = json.dumps(flower_dict,sort_keys=False)
with open(args.train_url+'label.json','w') as f:
    f.write(flower)
for i in range(np.size(prediction)):
    #打印每张图像的预测结果
    print(pic_names[i],"预测为:"+flower_dict[prediction[i]])
