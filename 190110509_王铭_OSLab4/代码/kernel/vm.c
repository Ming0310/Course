...
mykvminit(){
  ...
  // uart registers
  mykvmmap(pt,UART0, UART0, PGSIZE, PTE_R | PTE_W);

  // virtio mmio disk interface
  mykvmmap(pt,VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);

  // PLIC
  mykvmmap(pt,PLIC, PLIC, 0x400000, PTE_R | PTE_W);

  // map kernel text executable and read-only.
  mykvmmap(pt,KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  mykvmmap(pt,(uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  mykvmmap(pt,TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
  return pt;
}
...
void
mykvmmap(pagetable_t pt,uint64 va, uint64 pa, uint64 sz, int perm)
{
...
void
digui(pagetable_t pagetable,int level) 
{
    for(int i = 0;i < 512;i++){
      pte_t* pte = &pagetable[i];
      if((*pte) & PTE_V){ // 只打印有效的PTE
        ...
        printf("%d: pte %p pa %p\n",i,pagetable[i],child);
        if(level > 0)
          digui((pagetable_t)child,level - 1);
    }
  }
}
